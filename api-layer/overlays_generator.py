...
class OverlaysOutputGenerator(AutomaticSourceOutputGenerator):
    ...
    def endFile(self):
        ...
        if self.genOpts.filename == 'xr_generated_overlays.hpp':
            ...
            file_data += self.writeOverlaysUnionStructFuncs()
    ...
    def outputLayerHeaderPrototypes(self):
        ...
        for xr_union in self.api_unions:
            ...
            generated_prototypes += 'bool OverlaysOutputXrUnion(XrGeneratedDispatchTable* gen_dispatch_table, const %s* value,\n' % xr_union.name
        ...
        for xr_struct in self.api_structures:
            ...
            generated_prototypes += 'bool OverlaysOutputXrStruct(XrGeneratedDispatchTable* gen_dispatch_table, const %s* value,\n' % xr_struct.name
            generated_prototypes += '                           std::string prefix, std::string type_string, bool is_pointer,\n'
            generated_prototypes += '                           std::vector<std::tuple<std::string, std::string, std::string>> &contents);\n'
            if xr_struct.protect_value:
                generated_prototypes += '#endif // %s\n' % xr_struct.protect_string
    ...
    # entry in the Overlays content vector which is used to record the data to a file.
    #   self            the OverlaysOutputGenerator object
    ...
    def writeExpandedMember(self, base_type, is_pointer, pointer_count, member_param, member_param_prefix, member_param_name, has_prefix, prefix_string1, prefix_string2, expand, indent):
        ...
        if member_param.name == 'next':
            ...
            member_string += self.writeIndent(indent)
            member_string += 'if (!OverlaysDecodeNextChain(gen_dispatch_table, %s%s, %s, contents)) {\n' % (derefernce_str,
                                                                                                           member_param_name,
                                                                                                           member_param_prefix)
            ...
            if member_param_struct:
                member_string += 'if (!OverlaysOutputXrStruct(gen_dispatch_table, '
            else:
                member_string += 'if (!OverlaysOutputXrUnion(gen_dispatch_table, '
    ...
    # Write the C++ Overlays function for every union and structure we know about.
    #   self            the OverlaysOutputGenerator object
    def writeOverlaysUnionStructFuncs(self):
        struct_union_check = '\n// Union/Structure Output Helper functions\n'
        for xr_union in self.api_unions:
            if xr_union.protect_value:
                struct_union_check += '#if %s\n' % xr_union.protect_string
            struct_union_check += 'bool OverlaysOutputXrUnion(XrGeneratedDispatchTable* gen_dispatch_table, const %s* value,\n' % xr_union.name
        ...
        for xr_struct in self.api_structures:
            if xr_struct.protect_value:
                struct_union_check += '#if %s\n' % xr_struct.protect_string
            struct_union_check += 'bool OverlaysOutputXrStruct(XrGeneratedDispatchTable* gen_dispatch_table, const %s* value,\n' % xr_struct.name
            struct_union_check += '                           std::string prefix, std::string type_string, bool is_pointer,\n'
            struct_union_check += '                           std::vector<std::tuple<std::string, std::string, std::string>> &contents) {\n'
            indent = 1
            ...
            if is_relation_group:
                for child in relation_group.child_struct_names:
                    ...
                    struct_union_check += 'const %s* new_value = reinterpret_cast<const %s*>(value);\n' % (
                        child, child)
                    struct_union_check += self.writeIndent(indent + 1)
                    struct_union_check += 'return OverlaysOutputXrStruct(gen_dispatch_table, new_value, prefix, type_string, is_pointer, contents);\n'
                    struct_union_check += self.writeIndent(indent)
                    struct_union_check += '}\n'
                    if child_struct.protect_value:
                        struct_union_check += '#endif // %s\n' % child_struct.protect_string
            ...
            if xr_struct.protect_value:
                struct_union_check += '#endif // %s\n' % xr_struct.protect_string
            struct_union_check += '\n'
        struct_union_check += 'bool OverlaysDecodeNextChain(XrGeneratedDispatchTable* gen_dispatch_table, const void* value, std::string prefix,\n'
        struct_union_check += '                            std::vector<std::tuple<std::string, std::string, std::string>> &contents) {\n'
        struct_union_check += self.writeIndent(1)
        struct_union_check += '(void)gen_dispatch_table;  // silence warning\n'
        struct_union_check += '    try {\n'
            ...
            if enum_tuple.name == 'XrStructureType':
                ...
                for cur_value in enum_tuple.values:
                    ...
                    if struct_define_name:
                        ...
                        struct_union_check += 'if (!OverlaysOutputXrStruct(gen_dispatch_table, reinterpret_cast<const %s*>(value), prefix, "const %s*", true, contents)) {\n' % (
    ...
    def outputLayerCommands(self):
        ...
        for x in range(0, 2):
            ...
            for cur_cmd in commands:
                ...
                if count == 0:
                    generated_commands += '        if (name == "%s") {\n' % cur_cmd.name
                else:
                    generated_commands += '        } else if (name == "%s") {\n' % cur_cmd.name
                count = count + 1
